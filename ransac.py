...
def apply_RANSAC(img1_kpts, img2_kpts, matching_kpt_pair_indices, ransac_params):
    ...
    for i in range(ransac_params['N']):
        ...
        linear_model = np.linalg.lstsq(sampled_img2_kpts, sampled_img1_kpts, rcond=None)[0]
        ...
        for indices in matching_kpt_pair_indices:
            if indices not in sampled_kpt_pair_indices:
                ...
                pt1_hypotesis = np.dot(linear_model.T, np.append(pt2, [1]))
    ...
    # Fit the model on all the inliers
    ...
    affine_matrix = np.linalg.lstsq(inlier_img2_kpts, inlier_img1_kpts, rcond=None)[0]
